...
def show():
    ...
    with col3:
        alert_type_filter = st.multiselect(
            "Alert Type",
            ["DDoS", "Malware", "Intrusion", "Port Scan", "Brute Force", "SQL Injection"],
            default=[]
        )
    ...
    with col2:
        ...
        if not filtered_alerts.empty:
            selected_alert_id = st.selectbox(
                ...
                filtered_alerts['Alert ID'].tolist(),
                index=0
            )
    ...
    with col1:
        ...
        fig = px.line(trend_data, x='Date', y=['Critical', 'High', 'Medium', 'Low'],
    ...
    with col2:
        ...
        fig = px.bar(response_data, x='Severity', y=['Avg Response Time (min)', 'Target (min)'],
                     title="Average Response Time vs Target", barmode='group')
        st.plotly_chart(fig, width='stretch')
...
def generate_sample_alerts():
    ...
    for i in range(50):
        alert = {
            ...
            'Source IP': f"{random.randint(1, 254)}.{random.randint(1, 254)}.{random.randint(1, 254)}.{random.randint(1, 254)}",
