...
def show():
    ...
    with col1:
        data_source = st.selectbox(
            "Data Source",
            ["Live Traffic", "Historical Data", "Test Dataset", "Synthetic Data"]
        )
    ...
    with col4:
        traffic_type = st.selectbox(
            "Traffic Type",
            ["All Traffic", "Normal Only", "Suspicious Only", "Malicious Only"]
        )
    ...
    with col1:
        ...
        missing_data = pd.DataFrame({
            'Feature': ['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol',
                        'packet_size', 'duration', 'flow_rate', 'tcp_flags', 'label'],
...
def show_feature_analysis():
    ...
    # Feature selection
    features = ['packet_size', 'flow_duration', 'flow_rate', 'packet_count', 'byte_count',
                'src_port', 'dst_port', 'tcp_flags', 'protocol_type', 'inter_arrival_time']

    selected_features = st.multiselect(
        "Select Features to Analyze",
        features,
        default=['packet_size', 'flow_duration', 'flow_rate']
    )
...
def show_flow_analysis():
    ...
    with col1:
        ...
        fig = px.histogram(
            x=flow_durations,
            nbins=50,
            title="Network Flow Duration Distribution (seconds)"
        )
    ...
    with col2:
        ...
        fig = px.histogram(
            x=flow_sizes,
            nbins=50,
            title="Network Flow Size Distribution (KB)"
        )
        fig.update_layout(showlegend=False)
        st.plotly_chart(fig, width='stretch')
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=timeline_df['Hour'], y=timeline_df['Normal Flows'],
                             mode='lines+markers', name='Normal', line=dict(color='green')))
    fig.add_trace(go.Scatter(x=timeline_df['Hour'], y=timeline_df['Suspicious Flows'],
                             mode='lines+markers', name='Suspicious', line=dict(color='orange')))
    fig.add_trace(go.Scatter(x=timeline_df['Hour'], y=timeline_df['Malicious Flows'],
                             mode='lines+markers', name='Malicious', line=dict(color='red')))

    fig.update_layout(
        title="Network Flow Activity Over 24 Hours",
        xaxis_title="Time",
...
def show_raw_data():
    ...
    with st.expander("Search & Filter Options"):
        ...
        with col3:
            protocol_search = st.selectbox("Filter by Protocol",
                                           ["All", "TCP", "UDP", "ICMP", "HTTP", "HTTPS"])
    ...
    if port_filter > 0:
        filtered_data = filtered_data[
            (filtered_data['src_port'] == port_filter) |
            (filtered_data['dst_port'] == port_filter)
            ]
    ...
    if not filtered_data.empty:
        selected_rows = st.dataframe(
            ...
            column_config={
                ...
                "packet_size": st.column_config.NumberColumn(
                    "Packet Size (bytes)",
                    format="%d"
                ),
    ...
    if not filtered_data.empty:
        with st.expander("Data Summary Statistics"):
            ...
            with col2:
                ...
                categorical_cols = ['protocol', 'classification'] if 'classification' in filtered_data.columns else [
                    'protocol']
