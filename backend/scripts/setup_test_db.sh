...
DEFAULT_PASSWORD="change_this_in_production" # pragma: allowlist secret
...
load_env() {
    ...
    # Determine script directory and load .env.test file
...
main() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}       PostgreSQL Test DB Setup         ${NC}"
    echo -e "${BLUE}========================================${NC}"
    ...
    echo "   ENVIRONMENT=test alembic upgrade head"
    echo ""
    echo "2. Run Tests:"
    echo "   pytest"
    echo ""
