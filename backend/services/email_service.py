...
class EmailService:
    ...
    async def send_verification_email(
        self, background_tasks: BackgroundTasks, email: str, user_name: str, verification_token: str
    ) -> None:
    ...
    async def send_password_reset_email(
        self, background_tasks: BackgroundTasks, email: str, user_name: str, reset_token: str
    ) -> None:
        """Send password reset email"""
        reset_url = f'{settings.FRONTEND_URL}/reset-password?token={reset_token}'

        template_data = {'user_name': user_name, 'reset_url': reset_url, 'expiry_hours': 1}
